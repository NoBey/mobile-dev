...
//前面原生js用的是touches,其实还有一个targetTouches，在这两个的输出结果是一致的。
   ...
   handleTouchMove(e) {
       const { startX, startY, width, height, px, py} = this.state;//取得初始坐标和屏幕可视宽高
       const left = parseInt(e.touches[0].clientX) - 48 <= 0 || parseInt(e.touches[0].clientX) >= width - 48 ? (parseInt(e.touches[0].clientX) - 48 <= 0 ? 0 : width - 48) : parseInt(e.touches[0].clientX)
       const top=  parseInt(e.touches[0].clientY) - 48 <= 0 || parseInt(e.touches[0].clientY) >= height - 48 ? (parseInt(e.touches[0].clientY) - 48 <= 0 ? 0 : height - 48) : parseInt(e.touches[0].clientY)
       this.setState({
  ...
  render() {
    ...
    return (
      <div style={{top, left, width: width/3, height: width/3}}
           className   ={styles.MDev}
